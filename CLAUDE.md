...
- Dark backgrounds (`bg-gray-950`, `bg-gray-900`) on admin/dashboard; gradients; drop shadows heavier than `0 1px 2px`. The H-2d-R1 shadow exemption is REVOKED (Cam 2026-09-16, H-2g): `home.module.css` now carries **zero** `box-shadow` — a card is separated by a 1px `--line` hairline and nothing else, and `grep box-shadow src/app/home.module.css` must stay empty. One exemption survives (Cam 2026-09-15, H-2f): the cream section hand-off dissolves on `/` (`.storyTop`, `.storyBottom`, `.priceTop`, `.priceBottom`) are the only allowed **page-background** gradients. The ban was never about scrims: a gradient that darkens a photo so text sits on it (`.storyShade`, `.dHd::after`) or shapes a device mockup's own hardware (`.laptopBase`) is not a background gradient and was never in scope — the previous wording said "the only allowed gradients" while three such rules were already live, which is the kind of false absolute that gets a real rule ignored
