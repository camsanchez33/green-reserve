...
Generated 2026-09-02 19:32 UTC · branch `main` · HEAD `a48ece7` · working tree **1 dirty file(s)**
...
- **BUG: orphan banner loops forever — PARTIALLY BUILT (b88c8bf), NOT YET** — `RUN_QUEUE.md:1326`
...
14. MP-4c — detail tab trim + snooze/source/closedReason UI (needs the — `RUN_QUEUE.md:829`
15. MP-5 — courses reshape: evidence on list rows (already computed, never — `RUN_QUEUE.md:831`
16. MP-6 — money reshape: Revenue problems pinned all-time + collected-basis — `RUN_QUEUE.md:835`
17. MP-7 — comms merge: Broadcasts composer into Messages (owner-only), — `RUN_QUEUE.md:839`
18. MP-8 — chrome + System: sidebar real links + self-fetched badges + — `RUN_QUEUE.md:843`
19. MP-9 — adopt the design system (was ADMIN_V4 V4-6, full spec in — `RUN_QUEUE.md:847`
20. MP-10 — server-side pagination (was ADMIN_V4 V4-4): inquiries, activity, — `RUN_QUEUE.md:853`
21. MP-11 — auth guard into the layout (was ADMIN_V4 V4-7): session resolved — `RUN_QUEUE.md:857`
22. MP-12 — split courses/[id] (was ADMIN_V4 V4-9): 1,900 lines / 52 useState — `RUN_QUEUE.md:874`
23. BOOKING WINDOWS (schema change, attended) — how far ahead each audience can see/book the tee sheet: — `RUN_QUEUE.md:879`
24. COURSE_LAYOUT_SPEC Phase L2 — booking page sells products: product selector on tee sheet, per-product slots/pricing/labels everywhere (big; answer the spec's OPEN QUESTION first) — `RUN_QUEUE.md:900`
25. COURSE_LAYOUT_SPEC Phase L3 — isolation tests + admin layout summary (small) — `RUN_QUEUE.md:901`
26. Tiny run: legal entity name fill-in (no migration) — replace the {{COMPANY_LEGAL_NAME}} placeholder in /terms + /privacy with "TheGreenReserve LLC" + formation state (CAM: confirm  — `RUN_QUEUE.md:923`
27. ONBOARDING_V2_SPEC Phase V13 — guided operator onboarding: Getting Started checklist derived from real state (verify/password/look around/review page/connect Stripe/check schedule) — `RUN_QUEUE.md:943`
28. ONBOARDING_V2_SPEC Phase V13b — request-changes v2: structured category form on the preview page, requests live ON the inquiry (checkpoint area + addressable item list → "Send upda — `RUN_QUEUE.md:945`
29. BIRDIE_AI_SPEC Phase B1 — Birdie assistant foundation + operator helper: /api/birdie/chat (Anthropic API, Haiku, streaming), persona/tools derived server-side from surface+session, — `RUN_QUEUE.md:1002`
...
- CAM: confirm the state before this runs — e — `RUN_QUEUE.md:923`
...
**Totals:** 121 done · 8 awaiting review · 1 in flight · 29 not started · 8 revise pages open · 15 ideas · 2 parked.
