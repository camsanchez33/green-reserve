...
Generated 2026-09-02 20:35 UTC · branch `main` · HEAD `96c0693` · working tree **2 dirty file(s)**
...
- **BUG: orphan banner loops forever — PARTIALLY BUILT (b88c8bf), NOT YET** — `RUN_QUEUE.md:1380`
...
|---|---|---|---|---|
| MP-0 — shell fixes (was ADMIN_V4 V4-1): MainOffset one-liner for /admin | 2026-08-29 | 3d | `7246a62` | `RUN_QUEUE.md:390` |
| MP-1 | 2026-08-29 | 3d | `41f5ea8` | `RUN_QUEUE.md:420` |
| MP-1b — HOTFIX after /gr-review MP-1, SHIPPED 4ef11dd. Box open until | 2026-08-29 | 3d | `4ef11dd` | `RUN_QUEUE.md:455` |
| MP-2 | 2026-08-29 | 3d | `958f229` | `RUN_QUEUE.md:496` |
| MP-2b | 2026-08-29 | 3d | `a134af5` | `RUN_QUEUE.md:533` |
| MP-2c | 2026-08-29 | 3d | `e5b5413` | `RUN_QUEUE.md:582` |
| MP-2d | 2026-08-29 | 3d | `22d0f68` | `RUN_QUEUE.md:630` |
| MP-2e | 2026-08-30 | 3d | `bf3bcb2` | `RUN_QUEUE.md:672` |
...
14. MP-4e — a swallowed re-submission still DISCARDS its payload. When a — `RUN_QUEUE.md:869`
15. MP-4d — Overview action queue still derives "waiting on us" itself: — `RUN_QUEUE.md:876`
16. MP-5 — courses reshape: evidence on list rows (already computed, never — `RUN_QUEUE.md:885`
17. MP-6 — money reshape: Revenue problems pinned all-time + collected-basis — `RUN_QUEUE.md:889`
18. MP-7 — comms merge: Broadcasts composer into Messages (owner-only), — `RUN_QUEUE.md:893`
19. MP-8 — chrome + System: sidebar real links + self-fetched badges + — `RUN_QUEUE.md:897`
20. MP-9 — adopt the design system (was ADMIN_V4 V4-6, full spec in — `RUN_QUEUE.md:901`
21. MP-10 — server-side pagination (was ADMIN_V4 V4-4): inquiries, activity, — `RUN_QUEUE.md:907`
22. MP-11 — auth guard into the layout (was ADMIN_V4 V4-7): session resolved — `RUN_QUEUE.md:911`
23. MP-12 — split courses/[id] (was ADMIN_V4 V4-9): 1,900 lines / 52 useState — `RUN_QUEUE.md:928`
24. BOOKING WINDOWS (schema change, attended) — how far ahead each audience can see/book the tee sheet: — `RUN_QUEUE.md:933`
25. COURSE_LAYOUT_SPEC Phase L2 — booking page sells products: product selector on tee sheet, per-product slots/pricing/labels everywhere (big; answer the spec's OPEN QUESTION first) — `RUN_QUEUE.md:954`
26. COURSE_LAYOUT_SPEC Phase L3 — isolation tests + admin layout summary (small) — `RUN_QUEUE.md:955`
27. Tiny run: legal entity name fill-in (no migration) — replace the {{COMPANY_LEGAL_NAME}} placeholder in /terms + /privacy with "TheGreenReserve LLC" + formation state (CAM: confirm  — `RUN_QUEUE.md:977`
28. ONBOARDING_V2_SPEC Phase V13 — guided operator onboarding: Getting Started checklist derived from real state (verify/password/look around/review page/connect Stripe/check schedule) — `RUN_QUEUE.md:997`
29. ONBOARDING_V2_SPEC Phase V13b — request-changes v2: structured category form on the preview page, requests live ON the inquiry (checkpoint area + addressable item list → "Send upda — `RUN_QUEUE.md:999`
30. BIRDIE_AI_SPEC Phase B1 — Birdie assistant foundation + operator helper: /api/birdie/chat (Anthropic API, Haiku, streaming), persona/tools derived server-side from surface+session, — `RUN_QUEUE.md:1056`
...
- CAM: confirm the state before this runs — e — `RUN_QUEUE.md:977`
...
- `08d1c47` 2026-09-02 — queue/spec update
- `7023411` 2026-09-02 — Fix the duplicate-intake guard: a duplicate is the same course, not the same person
- `0a62bd8` 2026-09-02 — queue/spec update
- `e651928` 2026-09-02 — MP-4c: the inquiry detail trims down and the growth columns go to work
...
- `7691996` 2026-09-01 — Close the cron fail-open: one shared bearer check across all five cron routes
- `703a7d0` 2026-09-01 — Status board: fix the Windows git calls, regenerate, and make it a per-run step

---

**Totals:** 124 done · 8 awaiting review · 1 in flight · 30 not started · 8 revise pages open · 15 ideas · 2 parked.
