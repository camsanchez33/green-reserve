...
// H-1 §4 "See it work": the booking-page demo, in demo mode — no network, no
// Stripe, no real course. It mirrors the course page's slot-row interaction
// (tap a time → the row expands in place → Reserve → reserved state) and the
// three white-label knobs (accent, photo). It is deliberately NOT a fork of
// CourseBookingClient: that component is 1,500 lines of real booking logic
// with fetches and Stripe; the spec's "must not import Stripe" is only
// guaranteed by keeping this standalone. Copy here must stay honest — no fee
// claims beyond the $1.50/player line the product actually shows.
...
export default function HomeDemo() {
  ...
  return (
    <div className={s.stage} style={{ '--course': accent } as React.CSSProperties}>
      <div className={`${s.device} ${s.fade} ${photo ? '' : s.nophoto}`} aria-label="Example course booking page — try it">
        <div className={s.dHd}>
