...
// cancellation) using the same refund/waitlist/email logic as the golfer flow.
export async function PATCH(req: NextRequest) {
  const session = await resolveDashboardSession();
  if (!session) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });

  const { id, action } = await req.json();
  if (!id || action !== 'cancel') return NextResponse.json({ error: 'Missing id or unsupported action' }, { status: 400 });
  ...
  const result = await performCancellation(id);
