...
- [ ] ONBOARDING_V2_SPEC Phase V3b — playability follows hole count: 9/18/27/36 layouts, three-9s naming + combos, 18+9, par per nine, rotating-nines flagged as build note (small, no migration)
- [ ] ONBOARDING_V2_SPEC Phase V3c — walkthrough refinements: resident-rate validation bug, merge residents+memberships into one "Memberships & passes" step (tier types incl. resident card, verification: automatic vs purchased card), explicit "no cancellation policy" choice (small/medium, no migration)
