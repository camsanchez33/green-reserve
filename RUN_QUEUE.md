...
- [ ] COURSES_SHEET_SPEC Phase CS-2 — /admin/courses becomes the sheet: one table, 'Getting live' + 'Live' sections, 8 fixed columns (course, operator, status = setup n-of-5 or health, activity, fees 30d, next touch, live since, open); Live/Not live/Archived control and sort control removed; Export CSV (no migration, medium; AFTER the reskin batch lands — U-A touches this file). Supersedes the 'Courses' board framing in UI_REVISE_SPEC U-A.
- [ ] COURSES_SHEET_SPEC Phase CS-3 — course page: Setup card (5 steps + the existing action that completes each) for getting-live courses; Next check-in schedule/log card (IC-2 cards with the check-in agenda) for live courses; check-ins in the Activity tab (no migration, medium; AFTER CS-1 and IC-2).
