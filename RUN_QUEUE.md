...
- [ ] ONBOARDING_V2_SPEC Phase V7 — multi-nine data model: par adapts to hole count (no overall par on 27/36), structured combo builder with per-combo notes, per-nine par, per-combo ratings + auto-summed combo yardages per tee set, flows through Sheet tab + draft build (medium, no migration)
- [ ] ONBOARDING_V2_SPEC Phase V8 — memberships & passes + facilities fixes: tier fee input bug + $32110.01 display bug (root-cause the money round-trip, add sanity test), "Other" tier type, per-round pricing built out (discount vs separate, weekday/weekend/twilight, cart), pro shop phone, club rental multi-select arrange + phone (small/medium, no migration)
- [ ] ONBOARDING_V2_SPEC Phase V9 — draft build reuses existing operator by email (never fails on it), stranded-operator visibility, alert() → inline banners on admin inquiry pages (small, no migration)
