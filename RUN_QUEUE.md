...
- [ ] AGREEMENT_SPEC Phase AG-1 — AgreementAcceptance + AgreementVersion tables, Course.legalName (SCHEMA CHANGE, attended, additive); agreement text moves to versioned Markdown in legal/documents/ with sha256 hashes (page + PDF render from one source, replaces the JSX page and CURRENT_AGREEMENT_VERSION); migrate the legacy timeline lines as legacy rows; hasAcceptedAgreement reads the table. SEQUENCE: run before the LQ-2 copy run so the fee wording is edited once, in Markdown.
