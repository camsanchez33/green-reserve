...
- [x] BUG (acaa21e + 4bdbb0f, option a, picked by Claude under 'keep getting shit done' —
...
- SITE + DASHBOARD CAMPAIGN — full spec in SITE_DASHBOARD_SPEC.md. Public site +
  ...
  **SD-1 RUNS BEFORE MP-1.** The admin campaign below is 13 runs on the console
  ...
  - [x] SD-5 (see git log; migrations booking_lifecycle + operator_session_version) — SHIPPED: walk-in/phone POST + tee-sheet Walk-in form, no-show / still coming / paid at counter, partial-party check-in (prorated charge + receipt), close-a-day (cancel + email + fee waived, per-booking failures reported), operator session versioning. NEEDS REVIEW + a counter walk (walk-in → check in; partial headcount; close a day on a test date). ORIGINAL: lifecycle states: walk-in/phone booking POST (the biggest
...
- [x] COURSE_LAYOUT_SPEC Phase L2 (schema dacf315 attended with Cam — migration product_scoping DEPLOYED; build fd59547; review fixes 7473180, REVIEWED 2026-09-15: spec 18/28 → all met after fixes incl. rating on slot cards and every golfer surface; security 1 HIGH fixed (draft build oversell — defaults now paused) + 4 medium fixed; admin-ux 2 high fixed; design 1 fix) — SHIPPED: TeeTimeSchedule.productId + TeeTime.productId (nullable); engine one slot per product per time; lib/schedule-conflict.ts (12/12) + 409 on both schedule routes; schedules editor picks the round, groups by product, shows the conflict; layout-tab nudge; draft build one default schedule per product; course page 'Which round' selector + slot labels; product label on booking emails, receipt, manage, check-in, operator + admin sheets, golfer account. NEEDS REVIEW + a live walk on a 27-hole test course. ORIGINAL (Cam 2026-09-15: RUN NEXT; the spec's open question was answered 2026-07-16) — booking page sells products: product selector on tee sheet, per-product slots/pricing/labels everywhere (big; answer the spec's OPEN QUESTION first)
- [x] COURSE_LAYOUT_SPEC Phase L3 (af2fdf8) — admin course detail Setup tab shows the
  configured layout (nines, products with nines + per-tee-set rating/slope, tee
  sets with per-nine yardage) or says it is a simple single layout; inquiry Sheet
  tab links to the built course; scripts/isolation-test.ts gains a layout section
  (B's nine/product never in A's responses; a product built with B's nine never
  carries it). NOT RUN here (needs a dev server + DB) — Cam: `npx dotenv -e
  .env.local -- npx tsx scripts/isolation-test.ts` against a Neon branch. NEEDS REVIEW.
...
- [x] BIRDIE_AI_SPEC Phase B1 (f023d05 + review fixes 7d684c2, shipped OFF per Cam 2026-09-15) — SHIPPED + REVIEWED (design CLEAN; spec 30 MET / 4 PARTIAL, all four closed; security 4 findings, 0 critical/high, all fixed; admin-ux 3, 0 blocking, all fixed): /api/birdie/chat (operator persona only, session-derived, Haiku 4.5 streamed, caps + kill switch, logged), lib/birdie knowledge pack + read-only course context + guardrails, BirdieWidget on the dashboard, /admin/system Birdie card, PASSWORD_CHECKLIST Phase 7c (create the key, set ANTHROPIC_API_KEY + BIRDIE_ENABLED=true). DEVIATION: B1 gives the model the course facts as context built by typed server functions rather than model-invoked tools (no tool loop; cheaper, nothing to misuse) — B4's confirm-actions will need real tools. REVIEWED 2026-09-15 — fixes in 7d684c2: the rate limiter fails CLOSED for birdie: keys (a DB error must not uncap paid spend), the hourly cap is keyed on the course so staff+owner really share 20/h, the cap is taken before the body is parsed, a missing/rejected key reaches a real message, the widget remembers OPEN so a dashboard navigation no longer shuts Birdie, Escape closes + focus + aria-live, the log stops truncating, ARCHITECTURE lists the route as operator-only, and the isolation test gained five cross-tenant checks (20/20). STILL NEEDS the Phase 7c walk once the key exists. ORIGINAL (Cam 2026-09-15: BUILD IT behind BIRDIE_ENABLED=false; ANTHROPIC_API_KEY to be added to Vercel later) — Birdie assistant foundation + operator helper: /api/birdie/chat (Anthropic API, Haiku, streaming), persona/tools derived server-side from surface+session, tenant isolation as law, scope guardrails + rate caps + BIRDIE_ENABLED kill switch, floating Birdie chat UI, operator how-to knowledge pack + read-only course awareness + deep links, NO writes (BIG, no migration; PREREQ: Cam adds ANTHROPIC_API_KEY to Vercel — full spec in BIRDIE_AI_SPEC.md; B2 golfer / B3 admin / B4 confirm-actions follow)
...
- [ ] UI REVISE — see UI_REVISE_SPEC.md (decision record 2026-09-04/05: two looks by audience, Clubhouse structure, homepage prototype approved). U-0 SHIPPED a3c1bea, H-1 SHIPPED 767d81c (boxes open until review + live walk). Reskin batch 2026-09-11 + behavior items B-1/2/4/5/6/8/10 MERGED TO MAIN 525b32a on 2026-09-14 at Cam's instruction (reviewed; live walk pending — see UI_REVISE_SPEC §3/§4 notes). H-2a shipped 57b9150 + webm a4fd2b1. Run order U-0 → H-1 → U-G → B-1 → B-2 → U-O → B-6 → U-A → U-M. Reskin runs are ZERO behavior; behavior items are separate. Doc file: commit, never discard.

- [x] INQUIRY_CALL_SPEC Phase IC-1 (5bd0a85) — SHIPPED via migration `calls` (additive, applied by the prod build's migrate deploy — Neon rehearsal waived by Cam). Lib + API + test script (12/12). NEEDS REVIEW; post-deploy: /api/health 200 + migrate status up to date. ORIGINAL: ONE `Call` model (kind discovery|checkin; inquiryId OR courseId) + CourseInquiry.callSkippedReason + Course.nextCheckInAt (SCHEMA CHANGE, attended, additive — also the schema for COURSES_SHEET_SPEC), src/lib/inquiry-call.ts agenda catalog + src/lib/inquiry-needs.ts 'still need from them' + queueSignal learns about calls + schedule/reschedule/log/skip_call actions + build gate (call_required 409). Decision record 2026-09-13 in the spec header; mockup = canvas 'Inquiries Call Sheet'.
- [x] INQUIRY_CALL_SPEC Phase IC-2 (4af4399) — SHIPPED: src/components/admin/InquiryCallCards.tsx (Set-up-call + Log-the-call), header call chip, build-gate notice (client pre-check + API 409), Lead 'From the call', Activity calls block, details API callAnswers + sheet hints, easternToIso. NEEDS REVIEW + the §6 browser walk (three fake inquiries). ORIGINAL: inquiry detail: Set-up-call card (date/time/length/direction/number, agenda pre-checked from form gaps, confirmation email, skip-with-reason) + Log-the-call card (outcome, answers per agenda item, notes, follow-up, 'Save + send pre-filled sheet') + call answers as hints on the course's details sheet + build gate UI (no migration, medium).
- [x] INQUIRY_CALL_SPEC Phase IC-3 (c2f1fc0) — SHIPPED: one fixed-column table with tbody sections, Next-call cell (all variants), Still-need chips computed by the list API, calls-this-week in the header line, Export CSV (?format=csv). NEEDS REVIEW + the §6 browser audit. ORIGINAL: /admin/inquiries becomes the sheet: fixed 7-column table (course, contact·phone, stage, next call, still need, in stage, inquired) inside the existing queue sections; Next call cell variants; 'calls this week' in the header line; Export CSV (no migration, medium). Supersedes the 'inquiries board' line in UI_REVISE_SPEC U-A — the U-A reskin restyles THIS table, it does not build a board.

- [x] UI_REVISE_SPEC H-2a (57b9150) — SHIPPED with Cam's clip (mp4 only, 3.8MB — a touch over the 3.5MB note; no webm yet). Desktop-only mount, plays on visibility, reduced-motion/saveData/error → poster. NEEDS REVIEW + a walk. ORIGINAL: homepage story looks like a video: looping muted <video> under the pinned beats (poster = current still, webm+mp4 in public/home/, play/pause on visibility, reduced-motion/saveData/error → poster, mobile never loads it). CAM: pick/supply the clip (8–15s loop, free license, ≤3.5MB) — run can merge before the clip exists (small/medium, no migration, no dependency).
- [x] UI_REVISE_SPEC H-2b (a8498fb) — SHIPPED: SeeItWork tabs + HomeDashboardDemo (laptop frame, staff look scoped to the screen, tiles recount on Check In, Block/Unblock). B-7 also shipped (see UI_REVISE_SPEC §4). NEEDS REVIEW + a walk. ORIGINAL: 'See it work' two tabs: What golfers see (today's HomeDemo) · What you see (new standalone HomeDashboardDemo: stats tiles, date strip, tee sheet rows with check-in + block interactions, shared accent swatch, laptop frame). RUN AFTER U-O O1 LANDS so it is built once in the new operator look (medium, no migration).

- [x] INQUIRY_CALL_SPEC Phase IC-4 (see git log) — SHIPPED: funnel strip + SEGMENT_HINT removed from /admin/inquiries; ?tab=<segment> still narrows the sheet and shows one 'Showing: <stage> · Clear' pill; header counts + invariant banner kept; FUNNEL_SEGMENTS untouched. Verify: Overview tile → narrowed sheet with pill → Clear → full queue.
- [x] COURSES_SHEET_SPEC Phase CS-1 (a95fd27) — SHIPPED: lib/course-setup.ts, lib/course-checkin.ts, lib/course-action-queue.ts, POST /api/admin/course-calls (schedule/reschedule/log/set_next), courses list API adds calls/setup/linkedInquiryId/inquiryCalls, go-live hook in both paths, check-in rows in the Overview amber queue, scripts/course-checkin-test.ts 13/13. NEEDS REVIEW. ORIGINAL: course-setup.ts (5 setup steps from existing fields) + course-checkin.ts (14d-after-go-live then every 90d; agenda in code; checkInSignal) + courses API adds calls/setup/nextCheckInAt + schedule_checkin/log_checkin/set_next_checkin actions + go-live hook sets nextCheckInAt + overdue check-ins in the Overview action queue (no migration, medium; AFTER IC-1).
- [x] COURSES_SHEET_SPEC Phase CS-2 (7ef3959) — SHIPPED: one table, Getting live above Live, eight fixed columns, Next-touch cell (discovery call / check-in signal / Set up call / Schedule links), health + type filters, Archived footer view, Export CSV (?format=csv). NEEDS REVIEW + the §4 browser audit. ORIGINAL: /admin/courses becomes the sheet: one table, 'Getting live' + 'Live' sections, 8 fixed columns (course, operator, status = setup n-of-5 or health, activity, fees 30d, next touch, live since, open); Live/Not live/Archived control and sort control removed; Export CSV (no migration, medium; AFTER the reskin batch lands — U-A touches this file). Supersedes the 'Courses' board framing in UI_REVISE_SPEC U-A.
- [x] COURSES_SHEET_SPEC Phase CS-3 (see git log) — SHIPPED: Setup card (5 steps + completing action each) for getting-live courses, components/admin/CourseCheckInCard.tsx (schedule / set a date / log / move) for live ones, calls listed in Records, course-detail API returns calls. NEEDS REVIEW + the acceptance walk (draft → live → +14d → schedule → log → +90d). ORIGINAL: course page: Setup card (5 steps + the existing action that completes each) for getting-live courses; Next check-in schedule/log card (IC-2 cards with the check-in agenda) for live courses; check-ins in the Activity tab (no migration, medium; AFTER CS-1 and IC-2).

- [x] LQ-2 COPY RUN (42b7f4d, 2026-09-14) — DONE: all six listed surfaces plus five the file did not list (social card image, operator welcome email, /for-courses meta description, the "Green fees go 100% to the course" trust line on /book and the course page). Review found three more operator-facing claims (Payments tab intro + stat subtitle, new-booking operator email) — fixed 072fa1b; zero banned phrases left in src (the operator-agreement's own allowed sentence 'not deducted from your listed prices' is the only grep hit); CURRENT_TERMS_VERSION → 2026-09-14; register PM-4/CC-6 → MET. NOTE: AG-1's "run before LQ-2" sequencing was read after this shipped — the sentences live in legal/LQ-2_FEE_COPY.md, so AG-1 copies them into Markdown rather than re-deciding them. NEEDS REVIEW. ORIGINAL: fee-flow DECIDED 2026-09-14: keep the direct-charge + application-fee structure, rewrite every fee claim. Exact sentences per surface in legal/LQ-2_FEE_COPY.md (HomeContent pricing sub + delete the LQ-2 placeholder + frozen comment, ForCoursesContent hero/stat/box/2 FAQs, /terms Service fee + bump CURRENT_TERMS_VERSION, /operator-agreement §1 + §2, Footer comment). Discloses the $0.50 membership-dues fee (L-4). Verify: grep the banned phrases → zero hits; read each sentence against checkin-booking.ts + stripe.ts. Then register PM-4/CC-6 → MET.

- [x] AGREEMENT_SPEC Phase AG-1 (53b7b16) — SHIPPED via migration `agreements` (additive, applied by the prod build). Documents in legal/documents/ (operator agreement verbatim; brand license + accuracy attestation are ⚖️ COUNSEL drafts written this run), lib/agreements.ts, agreement-gate.ts, /operator-agreement renders the Markdown, acceptance route writes rows, seed + backfill scripts (run after deploy). NEEDS REVIEW. ORIGINAL: AgreementAcceptance + AgreementVersion tables, Course.legalName (SCHEMA CHANGE, attended, additive); agreement text moves to versioned Markdown in legal/documents/ with sha256 hashes (page + PDF render from one source, replaces the JSX page and CURRENT_AGREEMENT_VERSION); migrate the legacy timeline lines as legacy rows; hasAcceptedAgreement reads the table. SEQUENCE: run before the LQ-2 copy run so the fee wording is edited once, in Markdown.
- [x] AGREEMENT_SPEC Phase AG-2 (see git log) — SHIPPED: Sign step in onboarding + /dashboard/sign, POST /api/operator/sign, PDFs (@react-pdf/renderer) to private Blob + emailed, admin Records table, checklists count documents. Drafts are never presented (lib/agreements draft detection) so today only the Operator Agreement is signable — the Brand License and Accuracy Attestation join the step the moment their counsel marker is removed. NEEDS REVIEW + the §4 walk (fake operator: scroll-gate, 3 rows, PDFs by email, admin tab, go-live). ORIGINAL: onboarding 'Sign' step: legal name, signer name/title, scroll-to-end panels, Operator Agreement + authority + brand license (marketing opt-out) + accuracy attestation in one POST /api/operator/sign with IP/UA; PDF per document to Blob + emailed to operator and hello@; admin Records tab lists acceptances with PDFs; checklist/reminder count 3 documents (no migration, medium-big). CAM + COUNSEL: brand-license and attestation text must exist in legal/documents/ (not draft) before this ships.
- [x] AGREEMENT_SPEC Phase AG-3 (see git log) — SHIPPED: lib/agreement-required (window, per-course re-acceptance, 428 gate on the 15 configuration routes), AgreementNotice banner→modal + dfetch 428 mapping, hourly day-0 notice (idempotent), seed counsel gate + 30-day window, courses sheet chips + Overview rows. NEEDS REVIEW + the dev walk (seed a bump with a 1-minute deadline → banner → modal → 428 → sign → cleared; public booking unaffected). ORIGINAL: version bumps: seed refuses reacceptRequired bumps without counselReviewed; day-0 notice email; dashboard banner → modal after 30 days; operator writes 428 agreement_required except sign/check-in/bookings; courses sheet + Overview show overdue; mark_live refuses on overdue (no migration, medium).

- [x] UI_REVISE_SPEC H-2c (49d3d7a, REVIEWED 2026-09-15: 11/11 code claims met, no findings; Cam confirmed the live walk) — SHIPPED: band + tee.jpg + README row gone, line moved under the steps title, all five remaining images opened and confirmed golf. Verify live: no tee.jpg request on /. ORIGINAL: KILL THE PHOTO BAND (small, no migration, RUN FIRST): the 'We set it up' band shows a WIND FARM (bad Unsplash id for TEE). Delete the section + tee.jpg + README row, move its headline into the 'Live in four steps' sub line, verify every remaining image id is actually golf (no `tee.jpg` request on `/` afterwards).

- [x] UI_REVISE_SPEC H-2d (b3e26cd + cd67d82, REVIEWED 2026-09-15: 60/71 met, one NOT MET fixed in ce5e7e5 — the hero device showed '$1.50/player booking fee', now the compact HomeDemo has no fee line and its total excludes the fee; .beats/.prog z-index 2; trust line 13px. Cam confirmed the live walk. Follow-ups queued below as H-2d-R1..R3) — SHIPPED: cream hero with the live booking device (HomeDemo compact) + tee-sheet card, ball-in-cup gone, story clip un-zoomed, cream→green dissolve; v2 story files committed b3e26cd. NEEDS REVIEW + the acceptance walk at 1440 and 390 + Lighthouse mobile vs H-1. ORIGINAL: CLEAN HERO (direction A, chosen by Cam 2026-09-14): cream hero, no photo, headline left + live booking device (HomeDemo hero variant) + static tee-sheet card right, lockup nav unchanged; story video v2 files swapped in + scroll-zoom removed from the video (the 'vibration'); cream→video top dissolve on the story section (medium, no migration, AFTER H-2c). Canvas: 'Homepage Hero Directions' board Main.


- [x] UI_REVISE_SPEC H-2d-R1 (bab429b, DECIDED by Cam 2026-09-15: keep the shadows, amend the rule — CLAUDE.md BANNED line now exempts the / device mockups) — ORIGINAL: DESIGN DECISION, Cam: device-mockup shadows vs the CLAUDE.md ceiling. src/app/home.module.css:46 (.heroDevice .device), :48 (.heroSheet), :108 (.device), :159 (.laptopScreen) all carry `0 30px 50px -30px` and larger layers since H-1; CLAUDE.md bans shadows heavier than `0 1px 2px`. Either amend the BANNED line to exempt the marketing device mockups, or flatten all four to `0 1px 2px rgba(0,0,0,.05)`. Do not fix one without the others (small, no migration).
- [x] UI_REVISE_SPEC H-2d-R2 (111959f, REVIEWED 2026-09-15: Cam confirmed the live walk) — SHIPPED: HomeDemo header image priority/eager when compact; mp4 source before webm (webm not re-encoded — Cam's asset task if wanted). NEEDS REVIEW + Lighthouse mobile vs H-1. ORIGINAL: hero perf follow-up: (a) src/components/home/HomeDemo.tsx:40 the device header image is `loading="lazy"` but above the fold in the hero — when `compact`, load it eagerly (`priority`) so it doesn't pop in or compete with the h1 for LCP; (b) src/app/HomeContent.tsx:193-194 the v2 story.webm (4.23 MB) is larger than story.mp4 (4.09 MB) and listed first, so desktop browsers fetch the bigger file — put mp4 first or re-encode the webm under the mp4 size. Verify with Lighthouse mobile vs H-1's numbers (small, no migration).
- [x] UI_REVISE_SPEC H-2d-R3 (07cf78e) — SHIPPED: all six edits; .dashStatus (home.module.css:188) still hardcodes #8A6116 — outside the listed lines, left for a later sweep. NEEDS REVIEW. ORIGINAL: H-2c/H-2d hygiene: delete dead `.btnLight` (home.module.css:23-24), the orphan `/* BAND */` comment (:217), collapse the two `.js .heroSheet` animation rules (:59-61) into one `animation: riseSheet .6s var(--ease) both .15s`, replace the hex literals at :49 and :56 with `--ok` / `--warn` / `--ink-3` (add --ok/--warn to the `.root` token block), and drop the stale HERO row from docs/design/README.md:13 (small, no migration).

- [x] INQUIRY_FORM_SPEC Phase IF-1 (52f7bc3 + review fixes 206969b, REVIEWED 2026-09-15: spec 35/39 → all met after fixes; security 0 crit/0 high, 4 medium fixed or queued; admin-ux clean; design 1 fix) — SHIPPED: ten-input form (Public/Semi-private/Private, booking method → currentBookingMethod, call-time chips → needsJson.callPreference), success screen + confirmation email point at the Calendly call link (SC-2 swaps in the per-inquiry invite), booking_today always on the agenda with 'They said: …', setup sheet asks its four branch questions inline (saved → call → old form answers → ask) and never hides a section, semi-private sheet path, admin Answers tab renders the new shape. Cam to walk: submit ×3 course types, open the confirmation email, walk a sheet with no needs + no call, semi-private sheet. ORIGINAL: the inquiry form asks only what the call can't (Cam 2026-09-15). Ten inputs: name, title, email, phone, course, town/state, course type, HOW THEY TAKE TEE TIMES TODAY (→ existing currentBookingMethod column), optional call-time preference (→ needsJson.callPreference), free text. CUTS the eight courseType branch questions (resident rates, memberships, rounds/month, public tee times, member count, outings, member booking, charges-members) — all are agenda items now. THREE DOWNSTREAM FIXES ARE PART OF THIS RUN, especially (b): the details sheet branches off inquiry answers today and must fall back to call answers, then to asking inline — never silently hide a section. Success screen + confirmation email point at the booking link. Small, no migration.
- [x] SECURITY (21c8d25, REVIEWED as part of the IF-1 security audit that raised it) — SHIPPED: ResubmitPayload.verified; email/phone only carried when the submitter used the email on file; admin resubmit card labels unverified payloads. ORIGINAL: (from the IF-1 review, pre-existing MP-4a): the duplicate-intake path on POST /api/inquiries lets an unauthenticated stranger who knows a course's name + city + state write a contact 'correction' (their own phone/email) onto that course's live inquiry as a resubmit event; the admin diff reads as 'the course corrected something' and apply_resubmit would redirect outreach to the attacker. Blind (identical response) and needs an admin click, so MEDIUM. Fix: label the resubmit diff in the admin UI as unverified public-form input, and only record contact-field changes (email/phone) when the submitter's email matches the inquiry's on-file email (src/app/api/inquiries/route.ts dedupe block; src/app/admin/inquiries/[id]/page.tsx resubmit card; RESUBMIT_FIELDS in lib/inquiry-status.ts). Small, no migration.

- [x] CALL_SCHEDULING_SPEC Phase SC-1 (32d985e + review fixes 5ea6455, REVIEWED 2026-09-15: spec 30/33 met, 1 partial + 1 DST bug fixed; security 0 crit/0 high, 2 medium + 4 low fixed; migration DEPLOYED — prisma migrate status up to date, /api/health ok) — SHIPPED: migration call_invites (additive), lib/call-availability.ts 22/22 tests, lib/google-calendar.ts (jose JWT, no googleapis), scripts/google-calendar-check.ts. CAM TO DO before SC-2 is useful: PASSWORD_CHECKLIST Phase 7b (service account, share the calendar, two Vercel env vars, run the check script). NEEDS REVIEW. ORIGINAL: availability engine + Google (SCHEMA CHANGE, attended): CourseInquiry.callInviteToken/SentAt/ExpiresAt + Call.bookedByCourse/gcalEventId (migration `call_invites`, additive); src/lib/google-calendar.ts via a SERVICE ACCOUNT on a shared calendar (freebusy read + event write); src/lib/call-availability.ts = broad CALL_WINDOWS in code MINUS Google busy MINUS booked Calls MINUS 12h lead, 30-min slots, 14-day horizon; pure + unit-tested. **CAM FIRST: create the service account, share the calendar with it, add GOOGLE_SERVICE_ACCOUNT_JSON + GOOGLE_CALENDAR_ID to Vercel and PASSWORD_CHECKLIST.md.** The run cannot do that part.
- [x] CALL_SCHEDULING_SPEC Phase SC-2 (fffb2cc + review fixes 722934f, REVIEWED 2026-09-15: spec 38/48 → all met after fixes; security 0 crit/0 high, 4 medium + 4 low fixed; design clean) — SHIPPED: lib/call-invite.ts (21-day token, auto-sent from POST /api/inquiries, confirmation button uses it, admin alert names a failed send), lib/ics.ts, /api/call/[token] GET+POST (grid from Google free/busy + scheduled calls, preference first; book in a transaction with 409 slot_taken; reschedule; cancel; Google event create/move/delete; timeline events; course email with .ics + Cam email; S5 fallback + alert), /call/[token] page (pick → confirm with editable phone / they-call → manage view), noindex, rate-limited, Nav hidden. NOTE: until PASSWORD_CHECKLIST 7b is done the page shows the honest 'can't show my calendar' fallback, never unverified slots. Token is randomBytes(24) hex rather than the spec's cuid (stronger). NEEDS REVIEW + the §4 real-inbox walk. ORIGINAL: the invite + the public booking page (no migration): sendCallInviteEmail auto-sent on inquiry submit (AUTO_SEND_CALL_INVITE flag); /call/[token] public page, noindex, rate-limited, slot grid in ET, preference-first, confirm → transaction re-check → Call row (bookedByCourse) → Google event; becomes the reschedule/cancel view; confirmation emails both ways with a .ics attachment. GOOGLE UNREACHABLE = NO SLOTS + reply-with-times fallback, never the raw window.
- [x] CALL_SCHEDULING_SPEC Phase SC-3 (07a8952 + review fixes 0348a9e, REVIEWED 2026-09-15: spec 15/20 → all met after fixes; security 0 crit/0 high, 2 medium + 4 low fixed; admin-ux 2 high fixed; design clean) — SHIPPED: send_call_invite action + 'Send a booking link' / Resend on the set-up-call card with the sent-not-booked state; sheet Next-call cell 'Invite sent · Nd ago' (warn at 5d) + 'they picked it'; queueSignal invite-cold rule (5+ days, pending/in_review, no scheduled/talked call) with tests; sendCallReminders on the hourly cron (23.5–24.5h window, RateLimit key as the once-only belt, results in the cron response); System page 'Call booking' card (CALL_WINDOWS read-only + configured calendar id or 'not configured'); admin responses strip callInviteToken. NOTE: no in-app cron heartbeat exists for any cron (System page says so) — the reminder logs to the cron response + console like the others. NEEDS REVIEW. ORIGINAL: admin side (no migration): 'Send a booking link' beside Set up call / Skip, with sent-not-booked state + resend; sheet's Next-call cell learns 'Invite sent · Nd'; an unanswered invite 5d+ becomes a yourMove signal; 24h reminder on the existing cron with heartbeat logging; CALL_WINDOWS shown read-only on the System page with the configured calendar id.

- [x] UI_REVISE_SPEC H-2e (01481f9, REVIEWED 2026-09-15: design audit clean; spec 27/27 met after fixes 47d9bb6 — nav blur now fades with the white, mobile menu closes when the bar hides, H-1 shrink waits for the bar, story bottom wash moved to the story END outside the pin + off on phones, .device near-cream fill recorded as deliberate. Cam to eyeball: beats vs the bottom wash at 1440, 390 walk) — SHIPPED: .root cream + per-section cream removed; .s cards #fff with --line border; marketing footer bg-paper + Operator login link; .storyBottom 18vh dissolve under the beats; Nav: transparent lockup-only over the hero on / (threshold = #top height − nav height, read on scroll/resize), bar + Operator login + List your course fade in past it (opacity/background only, motion-reduce renders the bar from the start, non-home pages always solid); How it works/Pricing/FAQ deleted. Untouched on purpose: H-1's 40px height shrink; .device/.dash/.heroSheet stay #F7F5EF (border + shadow, they read). NEEDS REVIEW + the acceptance walk at 1440 and 390. ORIGINAL: ONE GROUND, ONE LOGO (small, no migration). BUG: `.root` is WHITE while body/hero/courses/final are cream #F6F4EC, so every background-less section (See it work, Live in four steps, FAQ) shows white → the page bands white/cream/white/cream with hard seams (Cam's screenshots 2026-09-15). Fix: .root → cream, drop the redundant per-section cream, FLIP `.s` step cards to #fff in the SAME commit (they're cream-on-white today and vanish on a cream ground — grep the module for every #F6F4EC/#F7F5EF first), footer bg-white → cream, and add the story's bottom dissolve to match H-2d's top. NAV: transparent over the hero (lockup only, no bar), gains its blur background + Operator login + List your course once scrolled past the hero; How it works/Pricing/FAQ deleted; add Operator login to the footer too — the footer has none today, so logo-only would leave live courses no route to their dashboard.

- [x] UI_REVISE_SPEC H-2f (bab429b, CONFIRMED by Cam 2026-09-15 'Add the dissolve') — SHIPPED: .priceTop/.priceBottom 18vh cream dissolves under the pricing content, off <960px; CLAUDE.md gradient ban amended to name the four hand-off classes. NEEDS REVIEW + a look at 1440. ORIGINAL: CAM TO CONFIRM before running (2026-09-15, 'i like having the fade from slide to slide'): after H-2e the only hard section edges left on / are the two around the pine pricing slab (steps → .price and .price → FAQ); every other hand-off is cream-to-cream or already dissolved (story top 22vh, story bottom 18vh). Proposal: give .price the same treatment — an 18vh cream→transparent overlay at its top and bottom, under its content — so the page has one hand-off language. Requires amending the CLAUDE.md gradient ban to read 'gradients, except the cream section hand-offs on the homepage (H-2d §3 / H-2e §5 / H-2f)' in the same commit, or the design auditor flags it every review. Alternative Cam may prefer: leave the pricing slab as the one deliberate hard break (H-2e §4 calls it that) and close this item. Files: src/app/home.module.css (.price), src/app/HomeContent.tsx (two aria-hidden divs), CLAUDE.md BANNED line (small, no migration, zero behavior).
