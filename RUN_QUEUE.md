...
- [ ] ONBOARDING_V2_SPEC Phase V10 item 5 — dashboard early access: rename Building-stage action to "Send dashboard access" with look-around copy, draft banner on dashboard, verify dashboard renders sanely on draft course, go-live email skips credentials if already sent (small, no migration)
- [ ] ONBOARDING_V2_SPEC Phase V11 — operator first-login: remove fake instant-verify (dev scaffolding in prod!), auto-verify via tokened email links, status-aware fallback verify screen, guided set-password→Stripe→dashboard sequence, audit all auth flows for dev bypasses (medium, no migration)
- [ ] IMAGE STORAGE hardening (small/medium, no migration) — prerequisite: Cam creates the Blob store in Vercel (Storage → Blob → greenreserve-images) so BLOB_READ_WRITE_TOKEN exists. Then: (1) upload route validation — max size (~8MB), content-type allowlist (jpeg/png/webp), reject others with a clear inline error; (2) resize/compress on upload (sharp if already available, else client-side canvas downscale) — course photos capped at ~2000px wide so 12MB phone photos don't blow the perf budget; (3) replace-photo deletes the old blob (no orphan storage costs); (4) every course photo rendered via next/image or with explicit width/height + lazy loading (perf budgets); (5) graceful upload error states in dashboard + details sheet (no silent failures); (6) alt text field per photo (SEO + accessibility, feeds course pages)
- [ ] Small run: Stripe Express dashboard link for operators (no migration) — on the dashboard Settings "Stripe Payouts" card, when stripeAccountActive: "View payouts & balance →" button → POST /api/operator/stripe/dashboard-link → stripe.accounts.createLoginLink(accountId) → redirect (link is single-use, generate fresh per click; operator session required, own course only). ALSO investigate: Cam's screenshot shows dashboard status "Draft — will take you live shortly" while the inquiry/admin shows the course LIVE — find where the operator settings page reads status and fix the stale/mismatched source.
- [ ] ADMIN_V3 A2b: platform Stripe section on /admin/revenue (OWNER-only, no migration) — pulls LIVE from Stripe platform account: current balance, next payout date/amount, application fees collected (7d/30d). RECONCILIATION line: expected GR fees from bookings (existing A2 math) vs application fees Stripe reports for the same period — green check when they match, red delta with explanation when they don't ("expected $12.00, Stripe shows $6.00 — investigate 4 bookings"). "Open Stripe dashboard →" link. Cache Stripe responses ~5min (rate limits). This is the "are we actually getting our $1.50" alarm — the capability bug would have been caught by this on day one.
- [ ] ONBOARDING_V2_SPEC Phase V12 — course approval loop: "Approve my page" + "Request changes" (→ messages thread) on preview page + draft dashboard banner, approval-aware Next-step card, going live stays admin action (small/medium, no migration)
- [ ] Small run: sheet passes → membership tiers automatically (no migration) — verify/finish the V4 draft-build mapping: every tier the course entered on the details sheet ("Memberships & passes": name, type, annual fee, per-round pricing incl. weekday/weekend from V8) must exist as MembershipTier rows on the draft course — the operator should open Member Management and see their tiers already built. Sheet answers that don't map cleanly (e.g. punch cards) → build note + the tier wizard PREFILLS from the sheet data when adding manually. Rule: if the course typed it on the sheet, they never retype it in the dashboard.
- [ ] MULTI-COURSE OPERATORS (schema change, attended) — Course.operatorId has a UNIQUE constraint: one course per operator forever, which silently breaks draft-build reuse (V9's "attach course to existing operator" is impossible) and blocks real multi-course operators. Migration drops the unique constraint (keep an index); code sweep: every `course.findUnique({ where: { operatorId } })` → findFirst/findMany with explicit selection logic; operator dashboard needs a course switcher IF operator has >1 course (simple dropdown, default single-course behavior unchanged); draft-build guard becomes the V9-1b behavior for real (same-name active → offer Create anyway; otherwise attach). Follow CLAUDE.md shipping checklist — Neon branch + migrate dev.
